...
const ModifyProfileInputPage = ({ route }) => {
    const { title, nicknameContent, bioContent, tagContent=[], languageContent=[] } = route.params;
    ...
    const [isCheckedList, setIsCheckedList] = useState(languages.map(lang => languageContent.includes(lang)));
        ...
        <SafeAreaView style={ModifyProfileInputStyles.container}>
            ...
            {(nicknameContent || bioContent) && (
                <View style={ModifyProfileInputStyles.containerBackgroundWhite}>
                    <View style={ModifyProfileInputStyles.backgroundWhite}>
                        <Text style={ModifyProfileInputStyles.textTitle}>{title}</Text>
                        {nicknameContent && (
                            <TextInput
                                style={[ModifyProfileInputStyles.textInput, {color: CustomTheme.primaryMedium}]}
                        ...
                        {bioContent && (
                            ...
                            <TextInput
                                ...
                                maxLength={60}
                            />
                            <Text style={ModifyProfileInputStyles.textCount}>{bioInput.length} / 60</Text>
                            </>
                        )}
                    </View>
                </View>
            ...
            {tagContent.length > 0 && (
                ...
                <View style={ModifyProfileInputStyles.infoTextContainer}>
                    <InfoCircle />
                    <Text style={ModifyProfileInputStyles.infoText}>최대 3개까지 선택 가능</Text>
                </View>
                <View>
                    {hobbyRows.map((row, rowIndex) => (
                    <View key={rowIndex} style={ModifyProfileInputStyles.containerRow}>
