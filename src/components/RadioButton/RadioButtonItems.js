...
const RadioButtonItems = ({ value, children, disabled, containerStyle, color='#FFC0C0', borderColor='#FF3E3E' }) => {
  ...
  const radioButtonStyle = [
    styles.radioButtonCircle,
    {
      ...
      backgroundColor: disabled ? '#fff' : (isSelected ? color : 'transparent'),
    }
  ];

  return (
    <Pressable onPress={triggerRadioButton} style={[styles.radioButtonItemContainer, containerStyle]}>
      <View style={radioButtonStyle}>
        {isSelected && (
          <View style={styles.innerCircle(disabled ? CustomTheme.bgBasic : color)} />
        )}
      </View>
      {children && (
